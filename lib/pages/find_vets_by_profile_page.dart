...
class _FindVetsByProfilePageState extends State<FindVetsByProfilePage> {
  ...
  Future<void> _fetchNearbyVets(LatLng location) async {
    ...
    int radius = 3000;
    const int maxRadius = 50000;
    List<places.PlacesSearchResult> vets = [];
    ...
    while (radius <= maxRadius) {
      ...
      if (response.status == 'OK' && response.results.isNotEmpty) {
        vets = response.results.where((v) => v.rating != null && v.rating! >= 4.5).toList();
        if (vets.isNotEmpty) {
          break;
        }
      }
    ...
    if (vets.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No high-rated vets found nearby.')),
      );
      return;
    }
    ...
    LatLngBounds bounds = _calculateBounds([
      location,
      ...vets.map((v) => LatLng(v.geometry!.location.lat, v.geometry!.location.lng)),
    ]);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        ...
        if (mapController != null) {
          final update = CameraUpdate.newLatLngBounds(bounds, 250);
          ...
          child: Column(
            ...
            children: [
              ...
              if (hours != null && hours.isNotEmpty) ...[
                const SizedBox(height: 12),
                const Text("Orar cabinet:", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Se cauta cabinete in zona ta', style: TextStyle(color: Colors.white, fontSize: 20)),
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              TextButton.icon(
                                ...
                                label: const Text("Detalii", style: TextStyle(color: Colors.white)),
